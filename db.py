...
# ─────────────────────────────────────────────
# OTP — persistenti su SQLite (sopravvivono ai restart di Render)
# ─────────────────────────────────────────────

def otp_save(email, code, expiry, req_count, req_time):
    with get_db() as db:
        db.execute("""INSERT INTO otp_codes
                      (email, code, expiry, attempts, req_count, req_time)
                      VALUES (?,?,?,0,?,?)
