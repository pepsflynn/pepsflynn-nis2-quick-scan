...
<body>
    ...
    <div class="main-container">
        ...
        <div class="card" id="step1">
            ...
            <label>Dominio aziendale (per scan tecnici e verifica sito web)</label>
            ...
            <label>
                Indirizzo IP pubblico aziendale (per scan infrastruttura: RDP, VPN, database)
                <span style="color:#63b3ed; font-weight:normal; font-size:12px;">- Rilevato automaticamente</span>
            </label>
            <input type="text" id="public-ip" placeholder="es. 151.101.1.1">
            <small style="color:#a0aec0; display:block; margin-top:-5px; margin-bottom:15px;">
                L'IP è stato pre-compilato con quello della tua connessione attuale. 
                <strong>Se sei in ufficio o connesso alla VPN aziendale, va bene così.</strong>
                Se sei in smart working, sostituiscilo con l'IP pubblico della rete aziendale.
            </small>
        ...
        <div class="card hidden" id="step2">
            ...
            <p style="color:#a0aec0; margin-bottom:20px;">
                Questa verifica è composta da due test indipendenti per valutare la conformità NIS2 della tua email aziendale (Art. 21).
            </p>
            ...
            <div class="verification-section">
                ...
                <p style="color:#a0aec0; font-size:14px; margin-bottom:10px;">
                    Analizza in automatico i record DNS e la configurazione di sicurezza del server di posta.<br>
                    <strong>Non viene inviata alcuna email.</strong>
                </p>
            ...
            <div class="verification-section">
                ...
                <p style="color:#a0aec0; font-size:14px; margin-bottom:10px;">
                    Verifica che hai accesso reale alla casella email. Riceverai un codice monouso.
                </p>
            ...
            <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid rgba(43,108,176,0.3); text-align: center; color: #a0aec0;">
                <small>⚠️ Entrambi i test devono essere superati per procedere con la valutazione NIS2 completa.</small>
            </div>
    ...
    <script>
            ...
            fetch('https://api.ipify.org?format=json')
                ...
                .then(function(data) {
                    if (data.ip) {
                        ...
                        document.getElementById('public-ip').style.borderColor = 'rgba(43,108,176,0.6)';
                    }
                })
                .catch(function() {
                    document.getElementById('public-ip').placeholder = 'Inserisci IP pubblico aziendale';
                });
        });
        ...
        function verifyDNS() {
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    html += "<tr><td>Dominio Analizzato</td><td><strong>" + r.domain + "</strong></td></tr>";
                    html += "<tr><td>MX Record</td><td class='" + (r.mx_valid ? "ok" : "error") + "'>" + (r.mx_valid ? "✅ Configurato" : "❌ Non valido") + "</td></tr>";
                    html += "<tr><td>SPF (Anti-Spoofing)</td><td class='" + (r.spf_valid ? "ok" : "error") + "'>" + (r.spf_valid ? "✅ Protetto" : "❌ Vulnerabile") + "</td></tr>";
                    html += "<tr><td>DMARC (Policy Sicurezza)</td><td class='" + (r.dmarc_valid ? "ok" : "error") + "'>" + (r.dmarc_valid ? "✅ Attiva (" + r.dmarc_policy + ")" : "❌ Assente") + "</td></tr>";
                    html += "<tr><td>DKIM (Firma Digitale)</td><td class='" + (r.dkim_verified ? "ok" : "error") + "'>" + (r.dkim_verified ? "✅ Verificata" : "❌ Non trovata") + "</td></tr>";
                    html += "<tr><td>TLS (Crittografia)</td><td class='" + (r.tls_supported ? "ok" : "error") + "'>" + (r.tls_supported ? "✅ Supportata" : "❌ Non supportata") + "</td></tr>";
                    html += "<tr style='background:rgba(43,108,176,0.1);'><td><strong>Punteggio Tecnico</strong></td><td><strong>" + r.score + "/" + r.max_score + " - " + r.level + "</strong></td></tr>";
                    html += "</table>";
                    document.getElementById("dns-result").innerHTML = html;
                    dnsVerified = true;
                    document.getElementById("dns-result").insertAdjacentHTML("beforeend", "<span class='dns-verified' style='margin-top:10px;display:inline-block;'>✅ Test Tecnici Superati</span>");
                    checkBothTests();
                }
        ...
        function sendOTP() {
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    document.getElementById("otp-result").innerHTML = "<p class='ok' style='font-size:16px;'>✅ Un codice di verifica è stato inviato a <strong>" + email + "</strong>. Controlla la tua casella di posta.</p>";
                } else {
        ...
        function verifyOTP() {
            ...
            if (!code) { alert("Inserisci il codice di verifica ricevuto"); return; }
            ...
            .then(function(data) {
                if (data.verified) {
                    otpVerified = true;
                    document.getElementById("otp-result").innerHTML = "<span class='verified-badge'>✅ Identità Digitale Confermata</span>";
                    document.getElementById("otp-section").classList.add("hidden");
                    checkBothTests();
                } else {
                    document.getElementById("otp-result").innerHTML = "<p class='error'>❌ Codice errato o scaduto. Riprova.</p>";
                }
                btn.disabled = false; btn.textContent = "Conferma Codice";
            });
        }
...
def send_otp():
    ...
    try:
        msg = MIMEText(f"Il tuo codice di verifica NIS2 Compliance Tool e: {code}\n\nInseriscilo nel tool per completare la verifica dell'identita.")
        msg['Subject'] = 'Codice OTP - NIS2 Compliance Tool'
...
def scan():
    ...
    # Usa l'IP pubblico per gli scan infrastrutturali, altrimenti il dominio
